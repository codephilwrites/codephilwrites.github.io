# 👋🏻 **Phil Peters (he/him) - Head of Software Engineering**
<sub><sup> Last Updated: 04/08/2025 </sup></sub>


This is a living document that aims to reduce ambiguity, build trust, and make it easier to work with me. It’s part guide, part open door - especially useful if you’re new to working with me or if we’re trying to collaborate more deeply. I'm sharing this to be transparent, but also to make space for you to do the same.


---

## 👨🏻‍💻 About Me

I’m Phil - Head of Software Engineering and I am based on the edge of the New Forest. I live with my wife Emma and our two cats, Maggie and Matilda. I love learning, especially about history, and I’ve recently started learning piano so I can play songs from my favourite musicals 🎭. If you want to talk about *Les Mis*, *Blood Brothers*, or anything Musicals - I’m in.

I’m also a die-hard Kansas City Chiefs fan 🔴🟡 you may spot subtle hints about this when I work from home!

Professionally, I advocate for *DevOps as a culture, not just a toolchain*. I’m focused on building great developer experiences, sustainable engineering practices, and enabling organisations to move fast without breaking things that matter. I care deeply about strategy, automation, and helping engineers do their best work. My current focus is on establishing Specsavers’ Software Engineering capability by creating a clear, scalable operating model that enables our engineers to grow, develop, and deliver with confidence. I’m also leading on global governance for Software Engineering, ensuring we have the right principles, practices, and standards in place across regions. Alongside this, I’m accountable for the technical delivery of the Blended Experience Programme by guiding software engineering through complex transition states to ensure quality, consistency, and momentum.

The Specsavers behaviour I most identify with is **🏃🏻‍♂️ Courageous** - not just pushing for change, but helping others feel confident to come along for the ride.


---
## 💬 Communication Preferences

- 🕒 My, typical, core working hours are:
  - **Mon**: 07:30 - 17:00 (Forum 5)🏢
  - **Tue**: 07:30 - 17:00 (Forum 5)🏢
  - **Wed**: 08:30 - 17:30 (Forum 5)🏢
  - **Thu**: 07:30 - 17:00 (Forum 5)🏢
  - **Fri**: 09:00 - 16:30 (Remote)🏠

- 📍 I prefer:
  - In-person meetings on Mon-Thu
  - Virtual meetings on Fri
  - Teams over email 
  - Late meetings over early ones
- ✈️ Once a month (between March-September) I work from the La Villiaze office, in Guernsey. [Ask me when I'm next there and we can schedule some time](https://teams.microsoft.com/l/chat/0/0?users=phil.peters@specsavers.com&message=Hi%20Phil,%20when%20are%20you%20next%20in%20La%20Villiaze?). 
- 📧 If I message outside your hours, I don’t expect a response until you’re back online. I respect your boundaries and assume you’ll respect mine.
- 🕕 My Teams and Outlook Notifications automatically disable during my non-working hours. You are free to message me anytime that works for you, I commit to responding within working hours. 


### ✉️ How to Contact Me
- ☕ Book [a chat in Outlook](https://outlook.office.com/bookwithme/user/eafc1b0f303d4c0f992efe4cb382f271@specsavers.com?anonymous&ep=plink) - I'm always happy to chat.
- 💬 [Message me on Teams](https://teams.microsoft.com/l/chat/0/0?users=phil.peters@specsavers.com&topicname=Chat) (my preferred channel - I don’t check email as often).
- I'm structured in how I work, but flexible when it matters.

### 🔠 Acronyms I tend to use in chat
_What do I do with all the time I save you ask?_ - I create more acronyms...
- LGTM - Looks good to me
- YMMV - Your mileage may vary (outcomes can vary depending on circumstances, personal preferences, or luck)
- AFAIK - As far as I know
- IIRC - If I remember correctly
- EGF - Engineering Governance Framework (a Specsavers term)
- JSYK - Just So You Know
- AFK - Away From Keyboard
- FWIW - For What It's Worth

---

## 🧑🏻‍💼 What I Do & How I Think
My role is to grow and scale our internal engineering capability - building both the strategy and the environment that enables our people to deliver great software, while also defining how we effectively partner with vendors, and independant 3rd-parties, to complement and extend our delivery model.

Right now, I’m contributing to the evolution of our engineering capability at Specsavers by shaping and supporting a long-term strategy for how we build and scale our software delivery. That includes influencing direction in areas such as:
- **Practice** (craft, community, consistency)
- **Value stream engineering** (optimising how we deliver end-to-end)
- **Core engineering** (standards, tools, environments)
- **Innovation** (time, space and sponsorship to explore ideas)
- **Governance and metrics** (because we can’t improve what we can’t see)

I care deeply about platform thinking, developer enablement, and how teams can balance autonomy with alignment. I try to zoom between strategic and tactical levels - one minute I’m talking about engineering operating models, the next I’m in the weeds of a CI/CD design. I’m happiest at the intersection of **strategy and delivery** - connecting high-level goals to the day-to-day work of our engineers. I move between coaching, problem-solving, facilitating alignment, and telling stories that bring our technical vision to life.

---

## 🧭 My Leadership Philosophy

_I believe in creating clarity, removing friction, and enabling people to do their best work._

My leadership style is rooted in servant leadership and is outcome-oriented - I prioritise enabling and supporting my teams to thrive. That said, I’m not hands-off: I take clear ownership of outcomes, set direction with intention, and step in confidently when leadership and decision-making are needed. I don’t believe in micromanagement - I trust people to own their outcomes. My role is to remove blockers, ask useful questions, and help us stay aligned to our mission and values.

---

## 🛠 How I Make Decisions

I use a mix of data, context, and instinct. I tend to ask:
- Is this scalable and sustainable?
- Does this align with our direction and values?
- Who will this affect, and how can we include them?

I prefer consensus where possible, but I will make clear decisions when needed. I’m happy to take accountability and explain the rationale behind any decision I make.

---
## 💻 Working With Me When…

- **When I'm focused on strategy or planning:** I may be slower to respond. Feel free to follow up.
- **When things are moving fast:** I’ll be more direct and may prioritise speed over detail.
- **When I go quiet:** I’m likely thinking something through - reach out if you need me.

---

## 🔁 How I Give and Receive Feedback

- I give feedback with intent to help, not to judge.
- I’ll praise in public but give critique in private.
- If I don’t give feedback immediately, it’s not because I didn’t notice - I might just be choosing the right moment.
- If I ever give feedback that feels off - tell me. I’m always learning and improving.

---

## 🛣 What I'm Currently Focused On

- Aligning Specsavers’ engineering function to the Long-Term Framework
- Technical delivery of the Retail Platform in a sustainable way with knowledge interalised
- Technical delivery of the Clinical Platform ensuring an effective evolution from SOCRATES Cloud
- Global oversight for technical alignment of Clincal and Retail platform supporting applications
- Supporting our Supply Chain teams in the delivery of their technical platforms and initiatives
- Creating a DevEx strategy rooted in platform thinking
- Maintaining practices and principles to scale engineering excellence through the Engineering Governance Framework

---

## 💼 My Working Style

- I’m motivated by the *why*. I ask a lot of questions to make sure we're solving the right problem.
- I want the context, not just the conclusion. Share the thinking behind decisions - especially if you're asking for a steer.
- I prefer clarity over perfection - I’d rather see an early idea than a polished doc that’s too late to change.
- I prefer the problem over the solution - even if you’ve already picked a path, walk me through the decision.
- I like structured thinking but messy whiteboards - don’t be afraid to show half-formed ideas.
- I communicate best via diagrams, metaphors, and real-world parallels.
- I favour asynchronous tools and clear, complete communication. I’ll try to give you everything you need in one message.
- I default to optimism and forward momentum, but I value dissent and devil’s advocacy.

### ⚖️ Strength of Feeling Scale   

I sometimes use a **Strength of Feeling Scale Scale (1-10)** ⚖️ to signal how strongly I feel about something. A “2” means I’ll go with the flow. A “9” means I care a *lot* and might be harder to sway. If I add `4/10` to a message or converstion this is me telling you how strong my conviction is. 

---

## 🤝 How You Can Help Me

- I talk fast. If you need me to slow down or repeat something, just say so.
- I prefer feedback directly, and if it’s negative, a 1:1 is best.
- I value meeting agendas - they help me stay present and prepared.
- I live with tinnitus and often use music to help me focus. If I’m wearing headphones, tap me - I’ll let you know if I’m on a call.
- I’d rather know something that *might* change than not know it at all. I work better when I have the full picture, even if it’s messy.
- I challenge things I care about, but I value hierarchy. Let me know if I’m stepping outside my lane - I’ll appreciate the redirect.

---

## 🧠 What I Assume About Others

- We both want to make things better - and we’ll get further by working together.
- You’ll let me know if something’s off - `silence = agreement` in my book, so challenge me if needed.
- You have good intent, even if your message lands awkwardly - I’ll try to offer the same grace.

---

## 💪 Strengths

- I bring structure to complexity without killing creativity.
- I communicate clearly and can adapt messages for different audiences.
- I balance vision with pragmatism
- I’m comfortable thinking big and setting bold direction, but I’m equally focused on execution - turning vision into practical outcomes.
- I care deeply about people, quality, and delivery - in that order.
- I can articulate a vision *and* help people rally around it.

---

## 👎 Weaknesses

- I can become so passionate about a direction that I forget to revalidate the destination. Nudge me if that happens.
- I sometimes fixate on “the better way” at the cost of “the good enough for now”.
- I dislike inefficiency so much that I occasionally jump ahead of a process. Call me out if that affects team alignment.
- I can be impatient with unnecessary complexity or bureaucracy.
- I sometimes get lost in strategic work – nudge me if I go quiet on something operational.

---

## 🧔🏻‍♂️ Talk to Me About

### 👨🏻‍💼 Professionally:
- DevOps, Platform Engineering, and Developer Enablement  
- Engineering Leadership and Transformation  
- Engineering Strategy and Operating Models  
- Scaling Software Delivery and Continuous Improvement  
- Anything in the “DevOps x Org Design” space
- Growing high-trust teams  

### 🙋🏻‍♂️ Personally:
- Musical Theatre 🎭 (favourite scores, underrated shows, or dream roles)  
- NFL 🏈 - especially if you're brave enough to support another team  
- Cats 🐈 - Maggie and Matilda love the attention  
- Travel ✈️ - tell me your favourite destination, I’ll tell you mine



